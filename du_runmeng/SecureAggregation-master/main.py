...
# BETA = np.random.randint(100, 1000, size=size)
BETA = [1] * size
...
def init(user_ids: list) -> dict:
    ...
    entities["server"] = Server(PRECISION)
    ...
    with tqdm(total=len(user_ids), desc='Generating keys', unit_scale=True, unit='') as bar:
        for id in user_ids:
            ...
            entities[id] = User(id, pub_key, priv_key, PRECISION)
...
def advertise_keys(user_ids: list) -> bool:
    ...
    server_thread = Thread(target=server.signature_server.serve_forever)
...
def masked_input_collection(user_gradients: dict) -> bool:
    ...
    def fun(user):
        global BETA
        user.mask_gradients(user_gradients[user.id], BETA, server.host, server.masking_port)
    ...
    global BETA
    server.check_all_gradients(BETA)
...
def unmasking(shape: tuple) -> np.ndarray:
    ...
    if len(UnmaskingRequestHandler.U_5) >= t:
        ...
        for u in UnmaskingRequestHandler.U_5:
            thread = Thread(target=entities[u].listen_shared_b, args=[BETA, UnmaskingRequestHandler.U_5, server.broadcast_b_port, server.host, server.finish_port])
            thread.daemon = True
            thread.start()
        ...
        server_thread = Thread(target=server.finish_sevrer.serve_forever)
        server_thread.daemon = True
        server_thread.start()
        
        output = server.unmask(shape)
        print("{:=^80s}".format("Server Finish Unmasking")) 
        server.broadcast_output(output, server.broadcast_b_port)
    
        cnt = 0
        while len(FinishRequestHandler.U_6) != len(UnmaskingRequestHandler.U_5) and cnt < wait_time:
            time.sleep(1)
            cnt += 1
        
        logging.info(f"Finished Users:{','.join(FinishRequestHandler.U_6)} ")
        # if len(FinishRequestHandler.U_6) > t:
            
        return output

    else:
        # the number of the received messages is less than the threshold value for SecretSharing, abort
        logging.warning(f"Just received {len(UnmaskingRequestHandler.U_5)} shares")
        return None
...
if __name__ == "__main__":
    ...
    for u in U_2:
        ...
        # gradients = np.random.randint(1,10000,size=shape)
        user_gradients[u] = gradients
        # user_gradients[u] = (gradients * PRECISION).astype(int)
