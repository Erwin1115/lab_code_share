...
from utils_base import sigmoid,R
from utils_base import num_of_client
...
class ES:
    ...
    def forward_prop(self, parameters):
        ...
        a = 10
        # for i in range(128):
        #     enc_a = CSP_PA.encrypt(10)
        #     x = CSP_PA.decrypt(enc_a)
        ...
        # ex]-O)).T @ Wo) * Vt.T
        C3 = CSP_PA.encryMatrix(to_Wh_2)
        C3_ = CSP_PA.decryMatrix(C3)
        
        # print(f"Index:{index}\nenc_Wh:{Wh}\nenc_Wo{Wo}")
        return index, to_Wo, to_Wh_1, to_Wh_2, C3
