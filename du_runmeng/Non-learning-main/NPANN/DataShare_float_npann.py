...
from utils_base import mod_pow, CSP_PA
from utils_base import dataset_path, enc_data_path
from utils_base import Precision, Gama, batch_size
from MyMatrix_base import load_matrix_float
...
def encrypt_data(train_data,train_labels,test_data,test_labels):
    ...
    for i in tqdm(range(batch_size)):
        data = ((train_data[i].T) * Precision).astype(int)
        ...
        # print(enc_matrix.shape)
        list_ret = []
        for i in range(len(data)):
            list_ret.append(CSP_PA.encrypt(int(data[i])))
        enc_x = np.array(list_ret)
        # enc_x = CSP_PA.encryMatrix(data)
        
        enc_result.append((i, enc_x, label))
        a = 1
    end = time.time()
...
def verify_dec_data(enc_result, train_data):
    ...
    for i, enc_data, enc_label in tqdm(enc_result):
        ...
        for i in range(len(enc_data)):
            list_ret.append(CSP_PA.decrypt(enc_data[i]))
            # result = A_Matrix_[i] @ result
        enc_x = np.array(list_ret)
        # assert (enc_x).any() == train_data[i].any()
    end = time.time()
    print(f"Decrypting cost {end-start} seconds, each x costs {(end-start)/batch_size} seconds")
...
def test_enc_dec():
    ...
    # length = len(enc_data) *
    # print(f"{len(enc_data)} Bytes {len(enc_data)/1024}KB {len(enc_data)/(1024*1024)}MB {len(enc_data/(1024*1024*1024))}GB")
