...
import random
import threading
from utils_NPMML import sigmoid, R, num_of_client, Precision
from utils_NPMML import Gama, CSP_PA, Gama_
from utils_NPMML import epochs, batch_size

from DataShare_NPMML import load_data
...
# from utils_NPMML import compute_pool
...
class ThreadedTCPRequestHandler(socketserver.BaseRequestHandler):
    ...
    def update_parameters(self, received_parameters):
        ...
        delta_Wh, delta_Wo = result.get(timeout=3600) 
        # delta_Wh, delta_Wo = calculate_parameters(received_parameters, A_Matrix_[index])
        ...
        for j in range(R):
            ...
            if compute_acc_lock.acquire(False):
                try:
                    ...
                    print(f"-------Round {j} finished. Now accuracy: {acc} Loss:{loss} UsedTime:{used_time}s-------")
