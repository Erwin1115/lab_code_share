...
compress = lambda g,BETA:int((g.flatten() * BETA).sum())
# compress = lambda g,BETA:int(((g.flatten()).astype(int) * (10000) * BETA).sum())
# compress = lambda g,BETA:int( (int(g.flatten() * 10000) * BETA).sum() )
def flatten(Wh, Wo):
    h = Wh.astype(int).flatten()
    o = Wo.astype(int).flatten()
    c = np.concatenate((h,o))
    return c
...
class SIG:
    ...
    def gen(path: str = None, nbits=1024) -> tuple:
        ...
        Args:
            ...
            nbits (int, opt ional): the number of bit used in RSA. Defaults to 1024.
...
class SocketUtil:
    ...
    @staticmethod
    def recv_broadcast(port):
        # receive the packet size
        # data = sock.recv(1024)
        global broadcast_map
        while port not in broadcast_map.keys():continue
        return broadcast_map[port]
